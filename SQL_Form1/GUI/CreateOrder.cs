...
namespace SQL_Form1.GUI
{
    public partial class CreateOrder : Form
    {
        ...
        private void PopulateDGV()
        {
            ...
            string selectedProduct = cbxWeddingList.SelectedItem.ToString();
            string []selectedProductArray = selectedProduct.Split(',');

            product.GetProductsDetails(Convert.ToInt32(selectedProductArray[0]));
            List<Product> products = product.GetManageProductList();
            dgvCmb.Items.Clear();
        ...
        private void PopulateComboBox()  //Method that adds WeddingList details to a combo box that displays WeddingLists.
        {
            WeddingListDBAccess weddingListDBAccess = new WeddingListDBAccess(Db);
            weddingListDBAccess.GetWL();
            List<WeddingList> weddingLists = weddingListDBAccess.ListGetWeddinglist();
            foreach (WeddingList w in weddingLists)
            {
                cbxWeddingList.Items.Add($"{w.WeddingListNo},{w.ClientForename}, {w.ClientSurname}, {w.ClientContactNo}");
            }
        }
        ...
        private void btnConfirmOrder_Click(object sender, EventArgs e)
        {
            ...
            if (cbxWeddingList.Text == "")
            {
                accepted = false;
            }
            if (txtBoxFullName.Text == "" || txtBoxStaffName.Text.Length >= 50)
            {
                accepted = false;
            }
            if (accepted)
            {
                ...
                string[] wedding = cbxWeddingList.Text.Split(',');
                weddingListOrder.OrderCode = OrderCode;
                weddingListOrder.WeddingListNo = Convert.ToInt32(wedding[0]);
                weddingListOrder.CustomerName = txtBoxFullName.Text;
                weddingListOrderDBAccess.InsertWLOrder(weddingListOrder);
        ...
        private void btnGenralOrder_Click(object sender, EventArgs e)
        {
            GeneralOrder Go = new GeneralOrder();
            Go.Show();
            this.Hide();
        }
